...
async function fetchFarandula(): Promise<{ noticias: Noticia[]; errorMsg?: string }> {
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (cacheExpirado) {
      const { noticias, errorMsg: apiError } = await fetchFarandula();
      ...
      (async () => {
        try {
          const { noticias } = await fetchFarandula();
          const noticiasValidas = deduplicarNoticias(filtrarYLimpiarNoticias(noticias)).slice(0, 20);
