...
const configs = [
	{
		...
		fetchNoticias: () => fetchNoticiasNewsData('', { category: "top", size: 20 }),
		region: "noticias",
		maxRetries: 3,
		retryDelayMs: 2000,
	},
	{
		...
		fetchNoticias: () => fetchNoticiasNewsData('music', { category: "entertainment", size: 15 }),
		region: "música",
		maxRetries: 3,
		retryDelayMs: 2000,
	},
	{
		...
		fetchNoticias: () => fetchNoticiasNewsData('entertainment', { category: "entertainment", size: 15 }),
...
export async function GET() {
	...
	// Se dispara cada endpoint en paralelo, sin esperar a que uno termine para el otro
	const endpoints = [
		"/api/noticias?force=1",
		"/api/musica?force=1",
		"/api/farandula?force=1"
	];
	const baseUrl = process.env.VERCEL_URL ? `https://${process.env.VERCEL_URL}` : "http://localhost:3000";

	// Lanzar todas las requests en paralelo, sin throttling
